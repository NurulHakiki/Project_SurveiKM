...
@section('title')
    Admin - Induk Pertanyaan
@endsection

@section('content')
   <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Management Induk Pertanyaan</h5>
        </div>
        <div class="card-body">
...
@section('modal')
    @include('admin.induk_pertanyaan.modal-edit')
    @include('admin.induk_pertanyaan.modal-add')
@endsection

@section('script')
<script>
    var table;
    var modalAddInduk_Pertanyaan = new bootstrap.Modal(document.querySelector('#modal-add-induk_pertanyaan'), {
        keyboard: true
    });
    var modalEditInduk_Pertanyaan = new bootstrap.Modal(document.querySelector('#modal-edit-induk_pertanyaan'), {
        keyboard: true
    });

    function addPertanyaan() {
        let dataForm = $('#form-add-induk_pertanyaan').serialize();

        $.ajax({
            url: "{{ url('admin/induk_pertanyaan') }}",
            type: 'post',
            headers: {
            ...
            success: function(response) {
                ...
                $('#form-add-induk_pertanyaan')[0].reset();
    ...
    function getDataditInduk_Pertanyaan(id) {
        $.ajax({
            url: "{{ url('admin/induk_pertanyaan') }}" + '/' + id,
            type: 'get',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            ...
            success: function(response) {
                swal.close();
                $('#induk_pertanyaan_edit').val(response.induk_pertanyaan.induk_pertanyaan);
                $('#unsur_edit').val(response.induk_pertanyaan.kode);
                $('#no_urut_edit').val(response.induk_pertanyaan.urut);
                ...
                modalEditInduk_Pertanyaan.show();
            },
            error: function(xhr, stat, err) {
                swal.close();
                if (xhr.status == 500) {
    ...
    function updateInduk_Pertanyaan() {
        let dataForm = $('#form-edit-induk_pertanyaan').serialize();
        let id = $('#id_induk_pertanyaan_edit').val();
        ...
        $.ajax({
            ...
            success: function(response) {
                alertSuccess(response.message);
                $('#form-edit-induk_pertanyaan')[0].reset();
                modalEditInduk_Pertanyaan.hide();
                fetchData();
            },
            error: function(xhr, stat, err) {
                swal.close();
    ...
    function deleteInduk_Pertanyaan(id) {
        $.ajax({
            url: "{{ url('admin/induk_pertanyaan') }}" + '/' + id,
            type: 'delete',
            headers: {
    ...
    function fetchData() {
        table = $('#table').DataTable({
            ...
            scrollX: true,
            ajax: {
                url : "{{ url('/admin/induk_pertanyaan/get-list') }}",
                type: 'post',
                    ...
                    return `
                        <div class="d-flex justify-content-center">
                            <button 
                                ...
                                data-id="${row.id}" 
                                data-induk_pertanyaan="${row.induk_pertanyaan}" 
                            >
                                Ubah
                            </button> 
                            <button 
    ...
    $(document).ready(function() {
        ...
        $('#form-add-induk_pertanyaan').validate({
        ...
        $(document).on('click', '.btn-edit', function() {
            let id = $(this).data('id');
            getDataditInduk_Pertanyaan(id);
        });


        $('#form-edit-induk_pertanyaan').validate({
            submitHandler: function(form) {
                updateInduk_Pertanyaan();
            },
            rules: {
                urut_edit: {
                    required: true,
